...
from .edge_grid import EdgeGridLayout


@dataclass(frozen=True)
class LevelLayout:
    definition: LevelDefinition
    layout: EdgeGridLayout


class EdgeGrid3DConfig(ConfigBase):
    ...
    @classmethod
    def from_spec(cls, spec: ConfigSpec) -> "EdgeGrid3DConfig":
...
class EdgeGrid3DLayout(LayoutBase):
    ...
    def from_spec(
        ...
        levels = [
            LevelLayout(
                ...
                layout=EdgeGridLayout.from_spec(level.layout_spec, config),
            )
    ...
    def to_spec(self, config: EdgeGrid3DConfig, with_grid_numbers: bool) -> LayoutSpec:
        levels_spec = [
            {
...
def _validate_location(location, levels: List[LevelLayout], *, context: str) -> None:
    cells = levels[location.level.index].layout.cells
    height = len(cells)
